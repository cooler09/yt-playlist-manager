...
exports.handler = async (event, context, callback) => {
  ...
  await axios.post(`${baseUrl}/oauth2/v4/token?client_id=${client_id}&refresh_token=${refresh_token}&client_secret=${client_secret}&grant_type=refresh_token`)
    .then(async response => {
      let headers = {
        "headers": {
      ...
      await axios
        .get(`${baseUrl}/youtube/v3/playlistItems?part=contentDetails&playlistId=${destinationPlaylistId}&maxResults=50`,
          headers)
        .then(async response => {
      ...
      for (const item of videosToBeRemoved) {
        await axios
          .delete(`${baseUrl}/youtube/v3/playlistItems?id=${item}`,
            headers)
          .then(async response => {
      ...
      for (const playlist_id of playlists) {
        ...
        await axios
          .get(`${baseUrl}/youtube/v3/playlistItems?part=contentDetails&playlistId=${playlist_id}&maxResults=50`,
            headers)
          .then(async response => {
            ...
            for (const vid of response.data.items) {
              ...
              if (videoDate > currDate) {
                ...
                if (!found) {
                  ...
                  await axios
                    .post(`${baseUrl}/youtube/v3/playlistItems?part=snippet`,
                      {
                        snippet: {
                    ...
                    .then(response => {
                      ...
                      console.log(`Video: ${vid.contentDetails.videoId} failed`);
                    });
                } else {
